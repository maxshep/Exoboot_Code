...
def fxRunFindPoles(port, baud_rate):
    dev_id = fxOpen(port, baud_rate, 0)
    if (fxFindPoles(dev_id) == FxInvalidDevice):
...
def get_exp_num(num_cl_args, argv):
    ...
    if(not exp_num.isdecimal()):  # Filter out letters
        sys.exit(
            'Please choose an experiment between [0 - ' + str(MAX_EXPERIMENT) + ']')
...
def get_dev_num(num_cl_args, argv, exp_num):
    ...
    if(experiments[exp_num][2] > 1):
        print('Max number of devices for this experiment:',
              experiments[exp_num][2])
    else:
    ...
    if((dev_num < 1) or (dev_num > experiments[exp_num][2])):
        sys.exit('Please choose a valid number of devices.')
...
def main(argv):
    ...
    print('\nRunning Experiment [' + str(exp_num) +
          '] with [' + str(dev_num) + '] connected device(s)')

    scriptPath = os.path.dirname(os.path.abspath(__file__))
    fpath = scriptPath + '/flexseapython/com.txt'
    ports, baud_rate = loadPortsFromFile(fpath)
    baud_rate = int(baud_rate)
    print('Using ports:\t', ports)
    print('Using baud rate:', baud_rate)

    # Time to call the demo script:
    try:
        if(dev_num == 1):
            experiments[exp_num][0](ports[0], baud_rate)
        elif(dev_num == 2):
            experiments[exp_num][0](ports[0], baud_rate, ports[1])
